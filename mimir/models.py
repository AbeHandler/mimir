    ...
    def load_base_model_and_tokenizer(self, model_kwargs):
        ...
        if self.config.openai_config is None:
            ...
            if "silo" in self.name or "balanced" in self.name:
                ...
                # When using device_map, we need to send tensors to the device where the first layer is
                # balanced_low_0 places the embedding layer on cuda:0
                ...
                # For models with device_map, get the device of the first model layer
                self.device = next(model.parameters()).device
                print(f"Model distributed via device_map, using {self.device} for input tensors")
