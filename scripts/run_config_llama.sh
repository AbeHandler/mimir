...
if [ -n "$BISECTION_QUERIES_PER_TOKEN" ]; then
    export CUDA_VISIBLE_DEVICES=0,1 && PYTHONFAULTHANDLER=1 CUDA_LAUNCH_BLOCKING=1 BISECTION_QUERIES_PER_TOKEN=$BISECTION_QUERIES_PER_TOKEN MIMIR_DATA_SOURCE=mimirdata MIMIR_CACHE_PATH=mimrcache conda run --live-stream -n mimir python -X faulthandler run.py --config "$CONFIG_FILE"
