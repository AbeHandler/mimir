...
#   - self.device='cuda:1' is used for inference operations
#   - Therefore we expose GPUs 0, 1, and 2 (Blackwell) where:
#     * GPUs 0-1: Model loading via device_map
#     * GPU 1: Inference operations (appears as cuda:1)
#     * GPU 2: Blackwell GPU for training context
#
# Usage:
...
#   - Sets CUDA_VISIBLE_DEVICES=0,1,2 to expose three GPUs
...
echo "Using GPUs 0,1,2 for Llama model (device_map uses 0-1, inference on cuda:1, Blackwell GPU 2 available)"

# Pass BISECTION_QUERIES_PER_TOKEN to both conda run commands if set
if [ -n "$BISECTION_QUERIES_PER_TOKEN" ]; then
    export CUDA_VISIBLE_DEVICES=0,1,2 && BISECTION_QUERIES_PER_TOKEN=$BISECTION_QUERIES_PER_TOKEN MIMIR_DATA_SOURCE=mimirdata MIMIR_CACHE_PATH=mimrcache conda run --live-stream -n mimir python run.py --config "$CONFIG_FILE"
    ...
    export CUDA_VISIBLE_DEVICES=0,1,2 && MIMIR_DATA_SOURCE=mimirdata MIMIR_CACHE_PATH=mimrcache conda run --live-stream -n mimir python run.py --config "$CONFIG_FILE"
